...
export default function UiKitClient() {
  ...
  return <main className="ui-kit-page">
    <PageHeader eyebrow="DEVELOPMENT ONLY" title="Acaora UI Kit" description="共享组件、状态、设计 token 与可访问交互的开发验收页。" actions={<Button onClick={() => setToast(true)}><Save size={17} />显示 Toast</Button>} />
    <UiKitTokens />
    <section className="ui-kit-section"><h2>Buttons & badges</h2><div className="ui-kit-row"><Button>主要操作</Button><Button variant="secondary">次要操作</Button><Button variant="ghost">文字操作</Button><Button variant="danger" onClick={() => setDialog(true)}>危险操作</Button><Button loading>处理中</Button><Button disabled>已禁用</Button><IconButton variant="secondary" label="通知"><Bell size={18} /></IconButton></div><div className="ui-kit-row"><Badge>默认</Badge><Badge tone="info">信息</Badge><Badge tone="success">已同步</Badge><Badge tone="warning">离线</Badge><Badge tone="danger">失败</Badge></div><small>用 Tab 键走查这一行：每个控件都必须出现可见的 focus 环（--brand）。主按钮为 --brand 实底，圆角 --r-md，不是 pill。</small></section>
